...
pub enum Token {
    ...
    #[regex(r"[A-Z][a-zA-Z0-9]{0,7}", validate_identifier)]
    Identifier(String),
...
fn validate_integer(lex: &logos::Lexer<Token>) -> Result<i32, CustomError> {
    let slice = lex.slice();
...
fn validate_char_array(lex: &logos::Lexer<Token>) -> Result<String, CustomError> {
    ...
    Ok(slice)  // Store the entire array representation in CharArray as a String
}
